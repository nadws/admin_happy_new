...
addEvent: function(entry, opts) {
 ...
 var newEvent = {
   ...
   warna1: entry.warna1 || false,
   warna2: entry.warna2 || false,
   warna3: entry.warna3 || false,
...
$dummyRight.html(rightText);
 ...
 renderEvent: function(event) {
   // Create event node
   if (event.url && !event.disabled && !event.warna1) {
     var $event = $('<a/>').attr('href', event.url);
   } else {
     var $event = $('<div/>');
   }
   if (event.warna1 || event.warna2 || event.warna3 && !event.disabled) {
     var $event = $('<div/>');
   } else {
     var $event = $('<div/>');
   }
   ...
   .toggleClass('sked-tape__event--warna1', !!event.warna1)
   .toggleClass('sked-tape__event--warna2', !!event.warna2)
   .toggleClass('sked-tape__event--warna3', !!event.warna3)
